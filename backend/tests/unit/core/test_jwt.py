...
def test_refresh_token_type():
    token, _jti, expires = encode_token(subject="user-1", token_type="refresh", days=7)
    payload = decode_token(token)
    assert payload["typ"] == "refresh"
    assert expires is not None
    assert timedelta(days=6) < (expires - __import__("datetime").datetime.now(expires.tzinfo))
