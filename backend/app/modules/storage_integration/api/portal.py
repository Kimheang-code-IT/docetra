...
@router.post("/portal/google-drive-sync/sources", status_code=201, response_model=DataEnvelope)
async def create_drive_source(body: DriveSourceBody, db: AsyncSession = Depends(get_db), user: User = Depends(current_user)):
    require_permission(user, "portal.google_drive_sync.edit")
    return {"data": await drive_sync.create_drive_source(db, body.model_dump(exclude_unset=True), user)}


@router.post("/portal/google-drive-sync/sources/{source_id}/sync", status_code=202, response_model=DataEnvelope)
...
@router.get("/portal/google-drive-sync/jobs/{job_id}", response_model=DataEnvelope)
