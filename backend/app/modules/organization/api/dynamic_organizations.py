...
@router.get("/organizations/_meta/types")
...
@type_router.post("")
async def create(org_type: str, request: Request, db: AsyncSession = Depends(get_db), user: User = Depends(current_user)):
    payload = await request.json()
    return {"data": await _service(org_type).create(db, user, dict(payload), dict(payload))}


@type_router.post("/bulk-delete")
async def bulk_delete(org_type: str, body: dict, db: AsyncSession = Depends(get_db), user: User = Depends(current_user)):
...
async def update(org_type: str, entity_id: str, body: dict, db: AsyncSession = Depends(get_db), user: User = Depends(current_user)):
    return {"data": await _service(org_type).update(db, user, entity_id, dict(body))}


@type_router.delete("/{entity_id}")
async def soft_delete(org_type: str, entity_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(current_user)):
    return {"data": await _service(org_type).soft_delete(db, user, entity_id)}


@type_router.delete("/{entity_id}/purge")
async def purge(org_type: str, entity_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(current_user)):
    return {"data": await _service(org_type).purge(db, user, entity_id)}


@type_router.post("/{entity_id}/archive")
async def archive(org_type: str, entity_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(current_user)):
    return {"data": await _service(org_type).lifecycle(db, user, entity_id, "archived")}


@type_router.post("/{entity_id}/restore")
async def restore(org_type: str, entity_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(current_user)):
    return {"data": await _service(org_type).lifecycle(db, user, entity_id, "active")}
...
async def set_favorite(org_type: str, entity_id: str, body: dict, db: AsyncSession = Depends(get_db), user: User = Depends(current_user)):
    await _service(org_type).get_item(db, entity_id)
...
async def add_comment(org_type: str, entity_id: str, body: dict, db: AsyncSession = Depends(get_db), user: User = Depends(current_user)):
...
async def replace_attachments(
    ...
    await _service(org_type).update(db, user, entity_id, {"attachments": files})
    return {"data": files}
