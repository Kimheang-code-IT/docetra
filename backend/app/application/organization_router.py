...
from app.modules.organization.schema import OrganizationPayload
...
from app.modules.record.service import collaboration, router_for, type_access
...
async def _actor_officer_id(db: AsyncSession, user: User):
    return (await people.ensure_officer_for_user(db, user)).id


async def _share_builtin_types(db: AsyncSession, organization_id, officer_id) -> None:
    await type_access.share_builtin_types(db, organization_id, officer_id)


def _service(org_type: str) -> OrganizationCollectionService:
    if not is_valid_org_type(org_type):
        raise HTTPException(404, "Organization type not found")
    return OrganizationCollectionService(
        org_type=org_type,
        actor_resolver=_actor_officer_id,
        created_hook=_share_builtin_types,
    )


def _lookup_service(resource: str) -> OrganizationCollectionService:
    return OrganizationCollectionService(resource=resource, actor_resolver=_actor_officer_id)


sector_router = router_for("sector", "sectors", service=_lookup_service("sectors"))
purpose_router = router_for("purpose", "purposes", service=_lookup_service("purposes"))
...
# Preserve the legacy route order: sector, purpose, then dynamic organizations.
