...
CI order (`.github/workflows/ci.yml`): gitleaks → backend lint/unit/contract → frontend lint/typecheck/unit → live integration → Playwright → auto-merge `dev` into `main` → deploy to production on `dev`/`main` push (secrets in `docs/DEPLOYMENT.md`).
