...
- Records (dynamic): `/records/{typeCode}` for every active type — CRUD, lifecycle, stage, schema, counts, options, comments, activity, attachments; `/records/_meta/surfaces` for FE boot/menus; `/records/logs` for document-surface activity. See [`prompt/backend/07-dynamic-record-collections.md`](../backend/07-dynamic-record-collections.md).
