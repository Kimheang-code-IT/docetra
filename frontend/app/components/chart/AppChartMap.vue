...
onMounted(async () => {
    try {
        geojson.value = await $fetch('/data/kh.json')
...
const maxValue = computed(() => {
    ...
    return Math.max(...props.data.map(d => Number(d.value) || 0))
})

function getProvinceColor(value: number) {
    if (!maxValue.value || !value) return 'rgb(241, 245, 249)' // bg-slate-100
    const intensity = Math.min(0.9, Math.max(0.15, value / maxValue.value))
    return `rgba(59, 130, 246, ${intensity})` // primary color with intensity
}
        ...
        >
            ...
            >
                <g>
                    ...
                    <g v-if="showLabels" class="pointer-events-none select-none">
            ...
            <div class="absolute bottom-2 p-1 rounded-lg bg-background/80 backdrop-blur-sm border border-accented shadow-sm select-none">
                ...
                <div class="h-1.5 w-32 rounded-full bg-linear-to-r from-blue-50/50 via-blue-500 to-blue-900 border border-white/10 mb-1"></div>
...
.province-name-label {
    font-family: "Noto Sans Khmer", "Khmer OS Siemreap", "Inter", sans-serif;
    font-size: 10px;
    fill: #0f172a;
    paint-order: stroke;
    stroke: #ffffff;
