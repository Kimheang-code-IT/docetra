...
const canonical = organizationCanonicalPath(param.value, id.value)
if (canonical) {
  await navigateTo(canonical, { replace: true })
}

watch(param, (value) => {
  const next = organizationCanonicalPath(value, id.value)
  if (next) {
...
<template>
  <DocumentEntityDocumentView v-if="config" :key="pageKey" :config="config" />
</template>
